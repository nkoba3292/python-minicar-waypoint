...
v0.3: v0.2から車輪速センサを追加
- 車輪速センサの対応追加（74HC590パルスカウンタ）
- 速度制御置き換え（IMU加速度積分 → 車輪速度）
- 衝突判定置き換え（超音波のみ → パルス停止検出追加）

変更詳細:
  1. 5周期ごとにパルスカウントから速度計算（車輪直径40mm、PPR=1.0）
  2. 位置推定でIMU加速度積分の代わりに車輪速度×Yaw角を使用
  3. 目標速度>10 かつ パルス=0 で衝突検出
  4. 車輪速度なしの場合は従来のIMU加速度積分にフォールバック
"""
import os
import sys
...
# File: 02_IMUPLUS_togikai_sample_new_v0.3.py
# v0.3: 車輪速センサ統合版
...
WHEEL_DIAMETER = 0.058  # 車輪直径 [m] (58mm)
WHEEL_PPR = 1.0  # 1回転あたりのパルス数（RPR220が1箇所の場合）
...
try:
    ...
    while True:
        ...
        if wheel_counter is not None:
            ...
            # 5周期ごとに速度計算
            if wheel_cycle_count >= WHEEL_SAMPLE_CYCLES:
                current_time_wheel = time.time()
                _, _, current_total = wheel_counter.read_incremental()
                
                # パルス差分と時間差分
                delta_pulses = current_total - wheel_last_total
                delta_time = current_time_wheel - wheel_last_time
                ...
                if delta_time > 0:
                    ...
                    wheel_velocity_mps = distance_traveled / delta_time
                else:
                    wheel_velocity_mps = 0.0
                
                # 更新
                wheel_last_total = current_total
                wheel_last_time = current_time_wheel
                wheel_cycle_count = 0
                
                # デバッグ表示
                if DEBUG_MODE:
                    print(f"[車輪速] Δpulse={delta_pulses} Δt={delta_time:.3f}s v={wheel_velocity_mps:.3f}m/s")
                
                # ===== 衝突判定（パルス停止検出） =====
                # 条件: アクセルが0でない かつ パルスが0 かつ 前回速度が一定以上
                current_accel_duty = abs(togikai_drive.Accel.__dict__.get('last_duty', 0)) if hasattr(togikai_drive.Accel, '__dict__') else 0
                # 簡易判定: 目標速度が0でない状況でパルスが0
                if target_speed > 10 and delta_pulses == 0 and wheel_velocity_mps < 0.01:
                    if not collision_detected:
                        collision_detected = True
                        print("[衝突検出] 車輪パルス停止！")
                        # 衝突時の処理はobstacle_avoidanceモードに任せる
                else:
        ...
        if position_tracking_enabled:
            # 1行に統合表示（車輪速度追加）
            if wheel_counter is not None:
                print(f'\rT:{race_time:.1f}s WP{idx}:({wp_x:.1f},{wp_y:.1f})m D:{distance_to_wp:.2f}m Y:{current_yaw:.0f}° V:{wheel_velocity_mps:.2f}m/s Line:{line_count} Fr:{FRdis:.0f} L:{LHdis:.0f} R:{RHdis:.0f} [{control_mode}]', end='', flush=True)
            else:
            ...
            if wheel_counter is not None:
                print(f'\rT:{race_time:.1f}s WP{idx} Y:{current_yaw:.0f}° V:{wheel_velocity_mps:.2f}m/s Line:{line_count} Fr:{FRdis:.0f} [{control_mode}]', end='', flush=True)
            else:
        ...
        if position_tracking_enabled:
            ...
            # ===== 車輪速度ベースの位置推定 =====
            if wheel_counter is not None and wheel_velocity_mps is not None:
                # 車輪速度を使用（Yaw角で方向を決定）
                yaw_rad = math.radians(current_yaw)
                
                # 車両前方方向を速度ベクトルとする
                # グローバル座標系: X軸=東、Y軸=北
                # Yaw=0°→北、90°→東、180°→南、270°→西
                velocity_x = wheel_velocity_mps * math.sin(yaw_rad)  # 東西成分
                velocity_y = wheel_velocity_mps * math.cos(yaw_rad)  # 南北成分
                ...
                delta_distance = wheel_velocity_mps * delta_time
                accumulated_distance += delta_distance
                
                if DEBUG_MODE and wheel_velocity_mps > 0.01:
                    print(f"[位置推定] v={wheel_velocity_mps:.3f}m/s Yaw={current_yaw:.0f}° Pos=({current_x:.2f},{current_y:.2f})")
            
            else:
                # 車輪速度が使用できない場合は従来のIMU加速度積分を使用
                # 静止判定閾値（ドリフト防止）
