...
v0.3からの継承:
  - 5周期ごとにパルスカウントから速度計算（車輪直径40mm、PPR=1.0）
...
ENABLE_LINE_DETECTION = False  # 白線検出機能有効化（False=一時的に無効化、センサ感度調整中）
...
try:
    ...
    while True:
        ...
        if wheel_counter is not None:
            ...
            pulse_delta_display = delta_pulses  # 表示用に保存
        ...
        if wheel_counter is not None:
            ...
            # 連続5ループ以上パルス0 かつ 速度<0.01で衝突判定
            if zero_pulse_count >= 5 and wheel_velocity_mps < 0.01:
        ...
        if position_tracking_enabled:
            # 1行に統合表示（車輪速度とパルス、モーター出力追加）
            if wheel_counter is not None:
                print(f'\rT:{race_time:.1f}s WP{idx} Pos:({current_x:.1f},{current_y:.1f})m D:{distance_to_wp:.2f}m Y:{current_yaw:.0f}° V:{wheel_velocity_mps:.2f}m/s Tgt:{target_speed:.0f} {wheel_pulse_info}Fr:{FRdis:.0f} [{control_mode}]', end='', flush=True)
            else:
                print(f'\rT:{race_time:.1f}s WP{idx}:({wp_x:.1f},{wp_y:.1f})m D:{distance_to_wp:.2f}m Y:{current_yaw:.0f}° Tgt:{target_speed:.0f} Line:{line_count} Fr:{FRdis:.0f} L:{LHdis:.0f} R:{RHdis:.0f} [{control_mode}]', end='', flush=True)
        else:
            if wheel_counter is not None:
                print(f'\rT:{race_time:.1f}s WP{idx} Y:{current_yaw:.0f}° V:{wheel_velocity_mps:.2f}m/s Tgt:{target_speed:.0f} {wheel_pulse_info}Fr:{FRdis:.0f} [{control_mode}]', end='', flush=True)
            else:
                print(f'\rT:{race_time:.1f}s WP{idx} Y:{current_yaw:.0f}° Tgt:{target_speed:.0f} Line:{line_count} Fr:{FRdis:.0f} [{control_mode}]', end='', flush=True)
