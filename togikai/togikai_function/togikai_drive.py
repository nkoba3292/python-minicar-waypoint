def ReadPWMPARAM(pwm):
    ...
    with open(path) as f:
        ...
        pwm.set_pwm(1, 0, PWM_PARAM[1][1])   # Ch.1=ESC, ニュートラル
        pwm.set_pwm(0, 0, PWM_PARAM[0][1])   # Ch.0=サーボ, 中央
        #print(PWM_PARAM)
    return PWM_PARAM
...
def Accel(PWM_PARAM,pwm,time,Duty):
    ...
    if Duty >= 0:
        ...
        pwm.set_pwm(1, 0, throttle_pwm)  # Ch.1=ESC
    elif Duty == 0:
        pwm.set_pwm(1, 0, THROTTLE_STOPPED_PWM)  # Ch.1=ESC
        time.sleep(0.01)
    else:
        #Need to Reverse -> Stop -> Reverse
        pwm.set_pwm(1, 0, THROTTLE_REVERSE_PWM)  # Ch.1=ESC
        time.sleep(0.01)
        pwm.set_pwm(1, 0, THROTTLE_STOPPED_PWM)  # Ch.1=ESC
        time.sleep(0.01)
